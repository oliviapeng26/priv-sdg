#!/usr/bin/env python3
"""SUPERSEDED -- iteration 1 of the TAPAS privacy evaluation. Use benchmark_tapas/.

    A single MIA + a single AIA against each generator. benchmark_tapas/ replaced
    this with the full 5-attack battery against all four generators under one
    shared threat model, and that is the only privacy result going into the
    write-up. Outputs live in results/tapas_LEGACY/.

    Not leaky, just narrower -- there is no methodological error here.
...
Reports: MIAttackReport, BinaryAIAttackReport, EffectiveEpsilonReport, ROCReport.
         Numeric summary → results/tapas_LEGACY/tapas_results.csv
         Plots / epsilon CSV → results/tapas_LEGACY/tapas/{method}/
...
RESULTS_DIR = Path('results') / 'tapas_LEGACY'
...
if all_mia_summaries:
    ...
    log.info('MIAttackReport saved to results/tapas_LEGACY/tapas/')

if all_aia_summaries:
    BinaryAIAttackReport(all_aia_summaries).publish(str(TAPAS_DIR))
    log.info('BinaryAIAttackReport saved to results/tapas_LEGACY/tapas/')

# ---- save numeric results ----
if rows:
    out = pd.DataFrame(rows)
    out.to_csv(RESULTS_DIR / 'tapas_results.csv', index=False)
    log.info(f'Saved results to results/tapas_LEGACY/tapas_results.csv')
else:
