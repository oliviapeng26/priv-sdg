...
file -- the earlier leaky-TSTR sweep, since deleted, wrote a different CSV, so
every setting starts as not-yet-done) and is
resumable: an already-recorded (method, n_iter, seed) row is skipped.

Meant to run on the Colab T4 (device auto-detected).

Run:
  python benchmark_tapas/tuning/convergence_check.py            # the full plan, both GANs
  python benchmark_tapas/tuning/convergence_check.py ctgan      # one method
  python benchmark_tapas/tuning/convergence_check.py --n-iter 50,100 --seeds 5
"""
...
#   2. (The earlier leaky-TSTR numbers behind the first CTGAN_N_ITER=50 /
#      DPGAN_N_ITER=100 choice were deleted; see git history.)
...
def run_one(method, n_iter, seed, fit_df, test_df, spec, trtr):
    ...
    # the generator was fitted on (the old evaluation_LEGACY/eval_synthcity_LEGACY.py, deleted; see git history).
    set_all_seeds(seed)
