...
WHAT IS HELD FIXED
    ...
    varies. The pools are therefore drawn exactly as run_dpgan_eps_sweep.py draws them,
...
OUTPUTS
    ...
    results/extras/dpgan_spike_diagnosis/signal_scan.csv         one row per eps
    results/extras/dpgan_spike_diagnosis/signal_scan_log.txt
...
def eps_slug(eps: float) -> str:
    """Matches run_dpgan_eps_sweep.eps_slug: 0.3 -> 'eps0.3', 3.0 -> 'eps3'."""
    return f"eps{eps:g}"
