...
Nothing outside `benchmark_tapas/` is modified, and
...
| file | what |
|---|---|
| `../tapas_wrappers/aim_generator.py` | `AIMGenerator` — SmartNoise AIM wrapped as a TAPAS `Generator` |
...
python benchmark_tapas/audits/run_aim_audit.py --probe 6

# 4. privacy, overnight in a screen                            (~12 h)
python benchmark_tapas/audits/run_aim_audit.py
```

Steps 1–2 fold AIM into `evaluation/results/{utility,fidelity}_summary.csv` automatically —
...
**ε=1.0 is already done** — it is the sample-size sweep in
`results/sample_size_sweep/aim/{50_100,200_500,500_1000,1000_2500}/`, which
`privacy_analysis.ipynb` reads. Its pool stays at `cache/aim_audit`; every other budget
gets `cache/aim_audit_eps{e}` and `results/eps_sweep/aim/eps{e}/{nt}_{nte}/`. The cache
names are asymmetric on purpose: renaming them would orphan a ~12 h pool. (ε=1.0 at
1000/2500 is the shared point of the sample-size sweep and the ε sweep. ε=100 was not run.)
...
```bash
python benchmark_tapas/audits/run_aim_audit.py --num-train 200 --num-test 500  # ~2.4 h
python benchmark_tapas/audits/run_aim_audit.py                                 # extends it
```
