...
as the four Synthcity methods. Three experiments run from this script (results go by experiment, not by script):

| experiment | what varies | fixed | results |
|---|---|---|---|
| counts sweep | `num_train`/`num_test` = 50/100 → 200/500 → 500/1000 → 1000/2500 | eps=1.0, epoch_cap=300 | `results/sample_size_sweep/dp_ctgan/{nt}_{nte}/` |
| eps sweep | `--epsilon` = 0.1, 10, 100 | 1000/2500, epoch_cap=300 | `results/eps_sweep/dp_ctgan/eps{e}/1000_2500/` (ε = 1 is the 1000/2500 stage of the counts sweep) |
| epoch_cap sweep | `epoch_cap` = 500, 750, 1000 | eps=100, 1000/2500 | `results/extras/dp_ctgan_epoch_cap/eps100_ep{cap}/1000_2500/` (cap 300 is `results/eps_sweep/dp_ctgan/eps100/1000_2500/`) |

| file | what |
|---|---|
| `../tapas_wrappers/dpctgan_generator.py` | `DPCTGANGenerator` — DP-CTGAN wrapped as a TAPAS `Generator` |
| `run_dpctgan_audit.py` | both sweeps, and `--probe N` for epochs-run / timing |

`dpctgan_generator.py` lives in `tapas_wrappers/`. One copy, deliberately: the epoch_cap
sweep is read against the epoch_cap=300 arm (`eps_sweep/dp_ctgan/eps100`), so both must be
fitting the same generator. The ε = 0.1 and 10 arms were run by an earlier script that is
now archived (`archive/dp_ctgan_sep2_attack_rerun/`); this script covers every arm through
`--epsilon`.
