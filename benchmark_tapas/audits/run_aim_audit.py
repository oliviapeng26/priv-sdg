...
Structurally this is run_synthcity_sample_size_sweep.py with one stage and a different
...
Nothing outside benchmark_tapas/ is modified. The AIM entry that would
...
THE EPSILON SWEEP
    ...
    WHERE eps=1.0 LIVES, AND WHY IT IS NOT REGENERATED
        ...
        results/sample_size_sweep/aim/{50_100,200_500,500_1000,1000_2500}/, and
        privacy_analysis.ipynb reads those paths. So eps=1.0 keeps its pool at
        cache/aim_audit, and only the new budgets are namespaced:
        cache/aim_audit_eps{e} and results/eps_sweep/aim/eps{e}/{nt}_{nte}/.

        The cache layout is asymmetric on purpose: uniform cache names would orphan
        a pool that cost ~12 h to build, in exchange for a tidier tree. The RESULTS
        tree follows the experiment instead (sample_size_sweep/ vs eps_sweep/), and
        eps=1.0 at 1000/2500 is the shared point between the two.
...
CHECKPOINTING, WHICH MATTERS MORE HERE THAN ANYWHERE ELSE
    run_dpgan_eps_sweep.py saves the threat model only after BOTH pools finish, which
...
OUTPUTS
    shared across the stages of ONE epsilon arm (the pool is grown, never rebuilt):
      ...
      results/sample_size_sweep/aim/aim_audit_log.txt   one log for every arm
    ...
    where cache_root is  aim_audit  and {results} is  results/sample_size_sweep/aim/{nt}_{nte}/
    at eps=1.0, and  aim_audit_eps{e}  /  results/eps_sweep/aim/eps{e}/{nt}_{nte}/  at
...
RUN IT IN THE RESTART LOOP, NOT DIRECTLY
    ...
    consecutive failures means something real is wrong, and the loop stops.
      ...
      run_stage () {
        fails=0
        while true; do
          python -u benchmark_tapas/audits/run_aim_audit.py --num-train $1 --num-test $2
          rc=$?
          [ $rc -eq 0 ] && return 0
    ...
    The epsilon sweep, one arm per night (each is a fresh ~12 h pool):
      ...
      run_stage () {            # $1 num_train  $2 num_test  $3.. extra flags
        fails=0
        while true; do
          python -u benchmark_tapas/audits/run_aim_audit.py \
...
Also:
  python benchmark_tapas/audits/run_aim_audit.py --probe 6                 # time 6 fits
  python benchmark_tapas/audits/run_aim_audit.py --probe 6 --epsilon 0.1   # noisiest arm
"""
...
AIM_DIR = RESULTS_DIR / "sample_size_sweep" / "aim"     # eps=1.0 stages + the shared log
AIM_EPS_DIR = RESULTS_DIR / "eps_sweep" / "aim"         # one eps{e}/ arm per other budget
...
def set_paths(epsilon: float) -> None:
    ...
    if epsilon == EPSILON:
        ...
        RESULTS_ROOT = AIM_EPS_DIR / eps_slug(epsilon)
...
# -- Copied from run_dpgan_eps_sweep.py, for the reason stated there ------------
