#!/usr/bin/env python3
"""TAPAS MIA audit of SmartNoise DP-CTGAN: the sample-size sweep, the eps sweep and
the epoch_cap sweep.

Structurally this is audits/run_aim_audit.py with a different generator, and it
is deliberately the same shape as the script that first ran the eps sweep
(archive/dp_ctgan_sep2_attack_rerun/run_dpctgan_audit_sep2.py). Everything load-bearing is reused
...
THE TWO EXPERIMENTS THIS SCRIPT RUNS
    counts sweep    eps=1.0, epoch_cap=300, num_train/num_test walking
                    ...
                    curve already (results/sample_size_sweep/); this puts DP-CTGAN on it.
...
WHERE THE POOL CACHE LIVES, AND THE TRAP THE COUNTS SWEEP FELL INTO
    ...
    audits/run_synthcity_sample_size_sweep.py is correct because it walks its stages upward on a
    ...
    THE eps SWEEP AND THE SAMPLE-SIZE SWEEP SHARE ONE POOL. At the default
    epoch_cap=300 the pool cache is cache/dpctgan_eps{e}, first grown by the eps
    sweep (now archived: archive/dp_ctgan_sep2_attack_rerun/) -- its eps=1.0 pool is
    already at 3500 fits, so the sample-size sweep re-used all of them and only
    re-ran the attacks. (That is why eps=1 and eps=100 exist as two attack runs in
    the archive: same synthetic data, different attack-training randomness.)

    Results go by experiment, not by script (see WHERE RESULTS GO below). The
    epoch_cap=300 arm at eps=100 is the eps_sweep/dp_ctgan/eps100 result; the epoch
    sweep reads it as 300, against 500/750/1000 under extras/dp_ctgan_epoch_cap/.
...
OUTPUTS
    shared across stages of one arm (the pool is grown, never rebuilt):
      ...
      results/sample_size_sweep/dp_ctgan/dpctgan_audit_log.txt   one log for every arm
    ...
    WHERE RESULTS GO ({results} above), by experiment:
      eps=1.0, cap 300      results/sample_size_sweep/dp_ctgan/{nt}_{nte}/
      other eps, cap 300    results/eps_sweep/dp_ctgan/eps{e}/{nt}_{nte}/
      cap != 300            results/extras/dp_ctgan_epoch_cap/eps{e}_ep{cap}/{nt}_{nte}/
...
RUN IT IN THE RESTART LOOP, NOT DIRECTLY
    ...
    while the pools are unfinished. Exit 0 means that stage is done.
      ...
      run_stage () {            # $1 num_train  $2 num_test  $3.. extra flags
        fails=0
        while true; do
          python -u benchmark_tapas/audits/run_dpctgan_audit.py \
...
Also:
  python benchmark_tapas/audits/run_dpctgan_audit.py --probe 3
  python benchmark_tapas/audits/run_dpctgan_audit.py --probe 3 --epsilon 100 --epoch-cap 1000
...
DPCTGAN_DIR = RESULTS_DIR / "sample_size_sweep" / "dp_ctgan"   # eps=1.0 stages + the shared log
DPCTGAN_EPS_DIR = RESULTS_DIR / "eps_sweep" / "dp_ctgan"       # one eps{e}/ arm per other budget
DPCTGAN_EPOCH_DIR = RESULTS_DIR / "extras" / "dp_ctgan_epoch_cap"
...
def eps_slug(eps: float) -> str:
    """Matches run_dpgan_eps_sweep.eps_slug: 1.0 -> 'eps1', 0.1 -> 'eps0.1'."""
    return f"eps{eps:g}"
...
def set_paths(epsilon: float, epoch_cap: int, cache_tag: str = "") -> None:
    ...
    epoch_cap == EPOCHS (300, the SmartNoise default) keeps the original pool cache,
    because that is where the 3500-fit pool already is -- see the pool notes in the
    module docstring. Any other cap is a different generator and gets its own pool
    and its own results tree.
    """
    ...
    if epoch_cap == EPOCHS:
        ...
        RESULTS_ROOT = (DPCTGAN_DIR if epsilon == EPSILON
                        else DPCTGAN_EPS_DIR / eps_slug(epsilon))
    else:
        CACHE_ROOT = CACHE_DIR / f"dpctgan_{eps_slug(epsilon)}_ep{epoch_cap}{suffix}"
        RESULTS_ROOT = DPCTGAN_EPOCH_DIR / f"{eps_slug(epsilon)}_ep{epoch_cap}"
...
def assert_pool_matches_counts(threat_model, num_train: int, num_test: int) -> None:
    ...
    That is why audits/run_synthcity_sample_size_sweep.py walks its stages UPWARD on a fresh pool:
