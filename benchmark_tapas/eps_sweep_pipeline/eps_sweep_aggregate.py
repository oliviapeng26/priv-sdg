...
WHERE EACH EPS COMES FROM
    eps in {0.1, 10, 100}   this sweep: results/eps_sweep/dpgan/eps{eps}/
    ...
    Mixing the two sources is safe precisely because run_dpgan_eps_sweep.py is
    run_synthcity_sample_size_sweep.py with the loop swapped: same background, same target,
...
Outputs:
    ...
    benchmark_tapas/results/eps_sweep/dpgan/eps_sweep_privacy_per_attack.csv  long form
...
COUNTS_DIR = RESULTS_DIR / "sample_size_sweep"
...
def eps_slug(eps: float) -> str:
    """Must match run_dpgan_eps_sweep.eps_slug / eps_sweep_generate.eps_slug."""
    return f"eps{eps:g}"
...
def load_meta(eps: float) -> dict:
    """Timings + guard fractions for one eps. Only run_dpgan_eps_sweep.py writes these."""
    if eps == REUSED_EPSILON:
        return {}
