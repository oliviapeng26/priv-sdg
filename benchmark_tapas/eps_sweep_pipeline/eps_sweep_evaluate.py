...
THE METRIC DEFINITIONS ARE IMPORTED, NOT REIMPLEMENTED
    ...
    sweep's numbers drift away from evaluation/results/{utility,fidelity}_summary.csv while
    ...
    (benchmark_tapas/tuning/convergence_check.py already imports
...
EPS = 1.0 IS RE-SCORED, NOT RE-GENERATED
    ...
    same files evaluation/results/{utility,fidelity}_summary.csv were built from. Scoring is
...
Outputs:
    benchmark_tapas/results/eps_sweep/dpgan/utility_fidelity_per_run.csv
    benchmark_tapas/results/eps_sweep/dpgan/utility_fidelity_summary.csv
    benchmark_tapas/results/eps_sweep/dpgan/generation_cost.csv   (eval rows upserted)

Run from the repo root, after eps_sweep_generate.py:
  python benchmark_tapas/eps_sweep_pipeline/eps_sweep_evaluate.py
  python benchmark_tapas/eps_sweep_pipeline/eps_sweep_evaluate.py --epsilons 10 100
  python benchmark_tapas/eps_sweep_pipeline/eps_sweep_evaluate.py --no-check
"""
...
def record_cost(row: dict) -> None:
    """Upsert one (epsilon, seed, stage) row into results/eps_sweep/dpgan/generation_cost.csv."""
    df = pd.DataFrame([row])
    if COST_CSV.exists():
...
def check_against_committed(summary: pd.DataFrame, tol: float = 1e-9) -> None:
    ...
    pairs = []
    for src, cols in ((REPO_ROOT / "evaluation" / "results" / "utility_summary.csv",
                       [f"tstr_{m}_auc" for m in UTILITY_MODELS]
                       + [f"trtr_{m}_auc" for m in UTILITY_MODELS]),
                      (REPO_ROOT / "evaluation" / "results" / "fidelity_summary.csv",
